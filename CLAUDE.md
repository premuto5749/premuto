...
**v2 핵심 개선사항**:
- 한 번의 검사에 여러 문서를 동시 업로드하여 처리 효율 극대화
...
1. **다중 파일 입력**: 사용자가 한 번의 검사에 해당하는 모든 문서(최대 10개)를 동시 업로드
   - 예: CBC.pdf, Chemistry.jpg, cPL_special.pdf
2. **병렬 OCR 처리**: 각 파일을 GPT-4o Vision API로 **동시 처리**하여 추출
   - 항목명, 결과값, 단위, 참고치 추출
   - 검사일자, 병원명 자동 추출 및 일치성 검증

#### Phase 2: 결과 우선 확인 (Preview Before Mapping)
3. **원본 데이터 미리보기**: OCR 추출 결과를 매칭 **전에** 먼저 사용자에게 제시
   - 목적: 숫자 오인식, 누락 등을 조기 발견
   - 사용자가 OCR 결과를 즉시 수정 가능

#### Phase 3: AI 기반 지능형 매핑
4. **컨텍스트 기반 매칭**: GPT-4o에게 다음 정보를 전달하여 휴리스틱 매칭 수행
   - DB의 전체 `standard_items` 목록
   - 기존 `item_mappings` 매핑 히스토리
   - OCR 결과 (raw_name, value, unit)
   - AI가 각 매칭에 대해 신뢰도 점수(0-100%) 반환

5. **불일치 항목 우선 처리**: 매칭 신뢰도가 낮은 항목을 상단에 배치하여 사용자가 집중적으로 검수

#### Phase 4: 검수 및 학습
6. **사용자 검수 및 승인**:
   - 🟢 신뢰도 높음: 원클릭 승인
   - 🟡 신뢰도 보통: 사용자가 확인 후 승인 또는 수정
   - 🔴 신뢰도 낮음: 수동 선택 또는 신규 항목 생성
7. **자동 학습**: 사용자가 승인/수정한 매핑을 `item_mappings`에 저장하여 다음번 자동 적용

#### Phase 5: 통합 저장
8. **일괄 트랜잭션**: 하나의 `test_records` + 다수의 `test_results`를 원자적으로 저장
   - 여러 파일에서 온 결과들이 하나의 검사 기록으로 통합
   - 각 결과에 출처 파일명과 매칭 신뢰도 함께 저장

#### Phase 6: 시각화
9. **피벗 테이블**: 날짜×항목 매트릭스로 트렌드 시각화
10. **시계열 그래프**: 주요 항목 클릭 시 모달 차트 표시
...
**v2 추가 원칙**:
- **배치 처리 우선**: 여러 파일을 개별 처리하지 않고 하나의 검사로 통합 처리
...
### C. 다중 파일 통합 규칙 (Multi-File Consolidation)
한 번의 검사에 여러 문서가 있을 때의 처리 규칙:

**규칙 1: 메타데이터 일치성 검증**
- 모든 파일에서 추출된 `검사 날짜`가 **동일**해야 함
- `병원명`이 다를 경우 사용자에게 확인 요청
- 불일치 발견 시 경고 메시지: "⚠️ 파일들의 검사 날짜가 다릅니다. 정말 같은 검사인가요?"

**규칙 2: 중복 항목 처리**
- 여러 파일에서 **같은 항목**이 추출된 경우 (예: WBC가 2개 파일에 모두 있음):
  - 값이 동일하면 → 1개만 저장
  - 값이 다르면 → 사용자에게 선택 요청 ("WBC 값이 2개 발견되었습니다. 어느 것을 사용하시겠습니까?")
...
**규칙 4: 원자적 저장**
- 모든 파일의 결과가 검수 완료될 때까지 DB에 저장하지 않음
- [모두 저장] 버튼 클릭 시 **하나의 트랜잭션**으로 처리
- 중간에 오류 발생 시 전체 롤백
