# Product Requirements Document: Premuto

**버전 이력**:
- v3.2: 검사항목 마스터 데이터 v3, 표준항목 관리, 대시보드 View 옵션, 티어 시스템, AI 설명 생성, 상세 내보내기, 카테고리 필터, 계정 관리
...
## 0. 일일 건강 기록 워크플로우 (Daily Log - v3.2 업데이트)
...
5. **카테고리 필터링** (v3.2 추가):
   - 타임라인 상단 카테고리 칩으로 필터링
   - 특정 카테고리 기록만 선택적 조회

6. **상세 Excel 내보내기** (v3.2 추가):
   - 기본 내보내기: 일일 요약만 포함 (제한 없음)
   - 상세 내보내기: 요약 + 개별 기록 시트 포함 (tier 월간 제한)
   - Free 티어: 1회/월, Basic 이상: 무제한
...
### 3.0 일일 기록 페이지 (/daily-log) - v3.2 업데이트
...
- 📅 **날짜 헤더**:
  ...
  - 📥 Excel 내보내기 버튼 (기본/상세 모드, v3.2 추가)
...
- 🏷️ **카테고리 필터** (v3.2 추가):
  - 카테고리 칩으로 필터링 (전체/식사/음수/약/배변/배뇨/호흡수)
...
### 3.9 설정 페이지 (/settings) - v3.2 업데이트
**목표**: 반려동물, 약, 테마, 계정, 데이터 관리를 탭으로 구성

**탭 구성** (5개):
- 🐾 **반려동물 (pet)**: 반려동물 프로필 추가/수정/삭제, 대표 사진
- 💊 **약 (medicine)**: 투약 프리셋 관리 (약 이름, 용량, 빈도)
- 🎨 **테마 (theme)**: 라이트/다크/시스템 모드 선택
- 👤 **계정 (account)** (v3.2 추가):
  - 카카오 계정 연동/해제
  - 비밀번호 설정 (소셜 전용 사용자)
  - 비밀번호 변경 (현재 비밀번호 확인 포함)
- 🗄️ **데이터 (data)** (v3.2 추가):
  - 커스텀 표준항목/별칭/매핑 현황 조회
  - 커스텀 데이터 초기화 (마스터 데이터로 복원)
